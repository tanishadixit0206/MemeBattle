import { Devvit, Participant, useForm, User, useState } from "@devvit/public-api";
...
import { ParticipantsDetails, PlayerDetails, RoomState, User_Details } from "../utils/types.js";
import { fetchRoomState, fetchUserDetails, myDetails, setCurrentCreatorinRedis } from "../redis/reddisFunctions.js";
...
export const GamePage = (context: Devvit.Context): JSX.Element => {
  ...
  const [mydetails, setMyDetails] = useState<User_Details | null>(null)
  const [iam, setIam] = useState<boolean>(false)
  ...
  async function sessionStart(roomID: string) {
    ...
    setRoom(room_1)
    const mydetails = await myDetails(context)
    ...
    if (room) {
      ...
      setMemeUrl(url_1)
      assignCreator()
      if (mydetails?.username === currentCreator) {
        setIam(true)
      }
    }
  }
      ...
      if (typeof values.text === 'string' && values.text.trim() !== '') {
        setMessages((prevMessages: any) => {
          const updatedMessages = prevMessages.length >= 5
            ? prevMessages.slice(1)
            : prevMessages;
          return [...updatedMessages, values.text + ` by :- ${currentCreator}`];
        });
      }
    }
  );
            ...
            {votingFormDisplayed ? <VotingForm users={mockUsers} />
              : <vstack height="100%" alignment="bottom center" padding="small">
                ...
                <hstack width="100%" alignment="center middle" >
                  {!iam ? <button size="small" icon="upvote-outline" appearance="success" onPress={() => { setVotingFormDisplayed(true) }}>Vote</button>
                    : null}
                  <spacer width="6%" ></spacer>
                  {iam ? <button size="small" icon="send" appearance="bordered"
                    onPress={() => {
                      context.ui.showForm(responseForm)
                    }}
                  >
                    Submit
                  </button> : <button size="small" icon="audience" appearance="bordered"
                    onPress={() => {
